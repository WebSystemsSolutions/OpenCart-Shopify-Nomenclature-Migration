<?php
class ModelCatalogCustomProduct extends Model {
    ...
    public function editProductJan($product_id, $jan)
    {
        $this->db->query("UPDATE " . DB_PREFIX . "product SET jan = " . (int)$jan . " WHERE `product_id` = " . (int)$product_id);
	...
	public function getProductsShopifyId() {
        $query = $this->db->query("SELECT jan FROM "  . DB_PREFIX . "product p");
//        var_dump(array_search('jan', $query->rows));
        $test = array();
