<?php
class ModelCatalogCustomManufacturer extends Model {
    ...
    $front_language_id = $language_info['language_id'];
        ...
        if ((isset($data['keyword'])) and (!empty($data['keyword']))) {
            ...
            $name_to_alias = $data['manufacturer_description'][$this->config->get('config_language_id')]['name'];
    ...
    public function getManufacturerName($manufacturer_id) {
        $query = $this->db->query("SELECT DISTINCT `name`, (SELECT keyword FROM " . DB_PREFIX . "url_alias WHERE query = 'manufacturer_id=" . (int)$manufacturer_id . "') AS keyword FROM " . DB_PREFIX . "manufacturer WHERE manufacturer_id = '" . (int)$manufacturer_id . "'");
